...
LogicalResult dropOutermostUnitDimsWithEncoding(RewriterBase &rewriter,
  ...
  linalg::ControlDropUnitDims options;
  options.controlFn = [](Operation *op) { return SmallVector<unsigned>{0}; };
  options.reshapableTypeFn = [](Type ty) { return isa<RankedTensorType>(ty); };
  options.collapseFn = [](RewriterBase &rewriter, Location loc, Value operand,
                          ArrayRef<int64_t> targetShape,
                          ArrayRef<ReassociationIndices> reassociation,
                          const linalg::ControlDropUnitDims &control) -> Value {
  ...
  options.expandFn = [](RewriterBase &rewriter, Location loc, Value result,
                        Value origDest,
                        ArrayRef<ReassociationIndices> reassociation,
                        const linalg::ControlDropUnitDims &control) -> Value {
