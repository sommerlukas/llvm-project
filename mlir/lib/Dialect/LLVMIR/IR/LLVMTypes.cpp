...
/// the list
/// extTypeParams ::= (type)? (',' type)* (',')? (integer)? (',' integer)*
static ParseResult
...
static constexpr llvm::StringRef spirvPrefix = "spirv.";
static constexpr llvm::StringRef armSVCount = "aarch64.svcount";

bool LLVM::LLVMTargetExtType::hasProperty(Property prop) const {
  // See llvm/lib/IR/Type.cpp for reference.
  uint64_t properties = 0;

  if (getExtTypeName().starts_with(spirvPrefix))
    properties |=
